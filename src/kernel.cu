...
void Boids::initSimulation(int N) {
  ...
  // Reverting to 0.55f for better flocking neighbor detection
...
__global__ void kernResetAndComputeIndices(
  int N, int gridCellCount, int gridResolution, glm::vec3 gridMin, 
  float inverseCellWidth, glm::vec3 *pos, int *indices, int *gridIndices, 
  int *gridCellStart, int *gridCellEnd) {
  int index = threadIdx.x + (blockIdx.x * blockDim.x);
  
  // Reset grid cell buffers (threads with indices < gridCellCount)
  if (index < gridCellCount) {
    gridCellStart[index] = -1;
    gridCellEnd[index] = -1;
  }
  
  // Compute indices for boids (threads with indices < N)
  if (index < N) {
    indices[index] = index;
    ...
    // Add debug output for the first few boids
    if (index < 5) {
      printf("Boid %d: pos (%f, %f, %f), grid (%d, %d, %d), gridIdx %d\n", 
             index, pos[index].x, pos[index].y, pos[index].z, gridX, gridY, gridZ, gridIdx);
    }
  }
}
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  if (index == 0 || cellIndex != particleGridIndices[index - 1]) {
    ...
    // Debug output for the first few grid cells
    if (index < 5) {
      printf("Grid cell %d: start index set to %d\n", cellIndex, index);
    }
    ...
    if (index > 0) {
      ...
      // Debug output for the first few grid cells
      if (index < 5) {
        printf("Grid cell %d: end index set to %d\n", particleGridIndices[index - 1], index);
      }
    }
  }
  ...
  if (index == N - 1) {
    ...
    printf("Last boid: Grid cell %d: end index set to %d\n", cellIndex, N);
  }
}
...
__global__ void kernUpdateVelNeighborSearchCoherent(
  ...
  glm::vec3 boidPos = coherentPos[index];
  ...
  for (int i = -1; i <= 1; i++) {
    for (int j = -1; j <= 1; j++) {
      for (int k = -1; k <= 1; k++) {
        int _x = min(max(x + i, 0), gridResolution - 1);
        int _y = min(max(y + j, 0), gridResolution - 1);
        int _z = min(max(z + k, 0), gridResolution - 1);
        int gridIndex = gridIndex3Dto1D(_x, _y, _z, gridResolution);
        if (gridCellStartIndices[gridIndex] != -1) {
          for (int h = gridCellStartIndices[gridIndex]; h <= gridCellEndIndices[gridIndex]; h++) {
            if (h != index) {
              float distance = glm::distance(coherentPos[h], boidPos);
              if (distance < rule1Distance) {
                percieved_center_of_mass += coherentPos[h];
                neighborCount1++;
              }
              if (distance < rule2Distance) {
                separate_vector -= (coherentPos[h] - boidPos);
              }
  ...
  if (neighborCount1 > 0) {
    ...
    v1 = (percieved_center_of_mass - boidPos) * rule1Scale;
  }
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  // Reset grid cell start/end indices and compute boid indices in a single kernel
  kernResetAndComputeIndices<<<fullBlocksPerGrid, blockSize>>>(
    numObjects, gridCellCount, gridSideCount, gridMinimum, gridInverseCellWidth,
    d_boidPositions, d_boidArrayIdx, d_boidGridIdx, d_gridCellStart, d_gridCellEnd);
  checkCUDAErrorWithLine("kernResetAndComputeIndices failed!");
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  // Step 1: Reset grid cell buffers and compute boid indices in a single kernel
  kernResetAndComputeIndices<<<fullBlocksPerGrid, blockSize>>>(
    numObjects, gridCellCount, gridSideCount, gridMinimum, gridInverseCellWidth,
    d_boidPositions, d_boidArrayIdx, d_boidGridIdx, d_gridCellStart, d_gridCellEnd);
  checkCUDAErrorWithLine("kernResetAndComputeIndices failed!");
  cudaDeviceSynchronize(); // Ensure all grid cells are reset before sorting

  // Step 2: Sort boids by grid cell index using Thrust (every frame for reliable flocking)
  // Restore sorting every frame for proper flocking behavior
  thrust::sort_by_key(d_thrust_boidGridIdx, d_thrust_boidGridIdx + numObjects, d_thrust_boidArrayIdx);
  checkCUDAErrorWithLine("thrust::sort_by_key failed!");
  ...
  kernIdentifyCellStartEnd<<<fullBlocksPerGrid, blockSize>>>(
    numObjects, d_boidGridIdx, d_gridCellStart, d_gridCellEnd);
  checkCUDAErrorWithLine("kernIdentifyCellStartEnd failed!");
  cudaDeviceSynchronize(); // Ensure cell indices are properly set
  ...
  cudaDeviceSynchronize(); // Ensure data is rearranged before velocity update
  
  // Step 5: Update velocities using coherent grid-based neighbor search (no shared memory)
  ...
  cudaDeviceSynchronize(); // Ensure velocity update is complete
  
  // Step 6: Update positions using the newly calculated velocities
  kernUpdatePos<<<fullBlocksPerGrid, blockSize>>>(
    numObjects, dt, d_sortedPositions, d_sortedVelocitiesB);
  checkCUDAErrorWithLine("kernUpdatePos failed!");
  cudaDeviceSynchronize(); // Ensure position update is complete
